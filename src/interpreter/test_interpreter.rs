...
// fn test_brackets_finder(src: &str, test_argument: Result<Vec<usize>, (ProgramCounter, &str)>) {
...
//         assert_eq!(Parser::find_brackets(src), Ok(expected_brackets));
//     } else {
...
//         ))
//         .unwrap();
//         let result = Parser::find_brackets(src).err().unwrap();
